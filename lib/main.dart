...
void main() async {
  try {
    ...
    try {
      ...
      // Логирование ошибки загрузки .env файла
      debugPrint('Could not load .env file: $e');
      debugPrint('Continuing without environment variables');
    }
...
class MyApp extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Используем MultiProvider для управления несколькими состояниями
    return MultiProvider(
      providers: [
        // Провайдер настроек (должен быть первым)
        ChangeNotifierProvider(create: (_) => SettingsProvider()),
        // Реактивная привязка настроек к ChatProvider
        ChangeNotifierProxyProvider<SettingsProvider, ChatProvider>(
          create: (_) => ChatProvider(),
          update: (context, settings, chat) {
            final instance = chat ?? ChatProvider();
            // При каждом изменении настроек обновляем API клиента
            instance.loadSettingsFromProvider(settings);
            return instance;
          },
        ),
      ...
      child: MaterialApp(
        ...
        // Основной экран приложения - теперь HomeScreen
