...
class ChatProvider with ChangeNotifier {
  ...
  // Метод для загрузки настроек из SettingsProvider
  void loadSettingsFromProvider(SettingsProvider settingsProvider) {
    final settings = settingsProvider.getCurrentSettings();
    updateApiSettings(
      settings['baseUrl'] as String,
      settings['apiKey'] as String?,
    );
    _log('Settings loaded from provider: Base URL: ${settings['baseUrl']}, API Key: ${settings['apiKey'] != null ? '***' : 'null'}');
  }

  // Метод для синхронизации настроек с API клиентом
  void updateApiSettings(String baseUrl, String? apiKey) {
    _api.updateSettings(baseUrl, apiKey);
    _log('API settings updated: Base URL: $baseUrl, API Key: ${apiKey != null ? '***' : 'null'}');
  }
  ...
  Future<void> sendMessage(String content, {bool trackAnalytics = true}) async {
    ...
    // Проверка наличия API ключа до начала отправки
    if (_api.apiKey == null || _api.apiKey!.isEmpty) {
      _log('API key not configured - blocking send');
      final errorMessage = ChatMessage(
        content: utf8.decode(utf8.encode('Error: API ключ не настроен. Откройте экран Настройки и введите ключ OpenRouter.')),
        isUser: false,
