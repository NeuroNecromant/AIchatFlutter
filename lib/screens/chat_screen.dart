...
class _MessageInputState extends State<_MessageInput> {
  ...
  void _handleSubmitted(String text) {
    ...
    if (settingsProvider.apiKey == null || settingsProvider.apiKey!.isEmpty) {
      // Показываем уведомление, если API ключ не настроен
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('API ключ не настроен. Перейдите в настройки.'),
  ...
  Widget build(BuildContext context) {
    return Consumer<SettingsProvider>(
      builder: (context, settingsProvider, child) {
        final hasApiKey = settingsProvider.apiKey != null && settingsProvider.apiKey!.isNotEmpty;
        
        return Container(
          margin: const EdgeInsets.symmetric(horizontal: 6.0),
          decoration: BoxDecoration(
            color: hasApiKey ? const Color(0xFF333333) : const Color(0xFF2A2A2A),
            borderRadius: BorderRadius.circular(16),
          ),
          child: Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  onSubmitted: hasApiKey && _isComposing ? _handleSubmitted : null,
                  decoration: InputDecoration(
                    hintText: hasApiKey ? 'Введите сообщение...' : 'API ключ не настроен',
                    hintStyle: TextStyle(
                      color: hasApiKey ? Colors.white54 : Colors.white38,
                      fontSize: 13,
                    ),
                  ...
                  style: TextStyle(
                    color: hasApiKey ? Colors.white : Colors.white54,
                    fontSize: 13,
                  ),
                ),
              ),
              IconButton(
                ...
                color: hasApiKey && _isComposing ? Colors.blue : Colors.grey,
                onPressed: hasApiKey && _isComposing 
    ...
    return ErrorBoundary(
      child: Scaffold(
        ...
        body: SafeArea(
          child: Column(
            children: [
              // Проверка API ключа в верхней части экрана
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Для работы с чатом необходимо настроить API ключ в настройках',
                                  style: TextStyle(
      ...
      builder: (context, chatProvider, settingsProvider, child) {
        ...
        return SizedBox(
          ...
          child: DropdownButton<String>(
            ...
            hint: Text(
              hasApiKey ? 'Выберите модель' : 'API ключ не настроен',
              style: TextStyle(
                color: hasApiKey ? Colors.white70 : Colors.white38,
                fontSize: 12,
              ),
            ...
            underline: Container(
              height: 1,
              color: hasApiKey ? Colors.blue : Colors.grey,
            ),
            onChanged: hasApiKey ? (String? newValue) {
              if (newValue != null) {
                chatProvider.setCurrentModel(newValue);
              }
            } : null,
            items: hasApiKey ? chatProvider.availableModels
        ...
        return Padding(
          ...
          child: Row(
            children: [
              ...
              Text(
                hasApiKey ? chatProvider.balance : '---',
                style: TextStyle(
                  color: hasApiKey ? const Color(0xFF33CC33) : Colors.white38,
                  fontSize: 12,
                ),
              ),
              const SizedBox(width: 8),
              ...
              const SizedBox(width: 4),
              Text(
                hasApiKey ? settingsProvider.provider : 'Не настроен',
                style: TextStyle(
                  color: hasApiKey ? Colors.white70 : Colors.white38,
                  fontSize: 10,
                ),
    ...
    return Consumer<SettingsProvider>(
      builder: (context, settingsProvider, child) {
        ...
        return PopupMenuButton<String>(
          icon: Icon(
            Icons.more_vert, 
            color: hasApiKey ? Colors.white : Colors.white38, 
            size: 16,
          ),
          color: const Color(0xFF333333),
          onSelected: hasApiKey ? (String choice) async {
          ...
          itemBuilder: (BuildContext context) => hasApiKey ? [
            ...
            const PopupMenuItem<String>(
              ...
              height: 40,
              enabled: false,
              child: Text('API ключ не настроен',
                  style: TextStyle(color: Colors.white38, fontSize: 12)),
            ),
          ],
        );
    ...
    return Consumer2<ChatProvider, SettingsProvider>(
      builder: (context, chatProvider, settingsProvider, child) {
        ...
        if (chatProvider.messages.isEmpty) {
          if (!hasApiKey) {
            // Показываем сообщение о необходимости настройки API ключа
            return Center(
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Для начала работы с чатом необходимо настроить API ключ в настройках',
            ...
            // Показываем стандартное сообщение о пустом чате
            return Center(
              child: Container(
                margin: const EdgeInsets.all(20.0),
                padding: const EdgeInsets.all(24.0),
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'API ключ не настроен. Перейдите в настройки для настройки ключа.',
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 12,
          ...
          child: Row(
            ...
            children: [
              _buildActionButton(
                ...
                icon: Icons.save,
                label: 'Сохранить',
                color: hasApiKey ? const Color(0xFF1A73E8) : Colors.grey,
                onPressed: hasApiKey ? () async {
                  final path =
              ...
              _buildActionButton(
                ...
                label: 'Аналитика',
                color: hasApiKey ? const Color(0xFF33CC33) : Colors.grey,
                onPressed: hasApiKey ? () => _showAnalyticsDialog(context) : null,
              ),
              _buildActionButton(
                context: context,
                icon: Icons.delete,
                label: 'Очистить',
                color: hasApiKey ? const Color(0xFFCC3333) : Colors.grey,
                onPressed: hasApiKey ? () => _showClearHistoryDialog(context) : null,
              ),
            ],
          ),
        );
